...
const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  const [selectedColor, setSelectedColor] = React.useState<string | undefined>(product?.colors ? product.colors[0] : undefined);
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={cn("sm:max-w-[800px] p-0 overflow-hidden", isMobile && "h-[90vh] overflow-y-auto")}>
        <div className={cn("grid md:grid-cols-2 gap-0", isMobile && "grid-cols-1")}>
          {/* Product Image Section */}
          <div className="relative bg-gray-100 p-8 flex items-center justify-center">
            <AnimatePresence mode="wait">
              <motion.img key={product.image} src={product.image} alt={product.name} className={cn("object-contain", isMobile ? "max-h-[180px]" : "max-h-[300px]")} initial={{
              opacity: 0,
              scale: 0.9
            }} animate={{
              opacity: 1,
            ...
            <div className="mt-4">
              ...
              {product.colors && product.colors.length > 0 && <div className="mt-5 md:mt-6">
                  ...
                  backgroundColor: color
                        ...
                        {selectedColor === color && <Check className="h-3 w-3 md:h-4 md:w-4 text-white mx-auto" />}
                      </button>)}
                  </div>
                </div>}
              ...
              <div className="mt-5 md:mt-6">
                ...
                <div className="flex items-center">
                  ...
                  <Button variant="outline" size="icon" onClick={incrementQuantity} disabled={quantity >= product.stock} className="h-8 w-8 md:h-9 md:w-9">
                    +
                  </Button>
              ...
              <div className="mt-6 md:mt-8 flex space-x-2">
                <Button className="flex-1 relative overflow-hidden text-sm md:text-base" onClick={handleAddToCart} disabled={product.stock === 0 || isAddingToCart}>
                  <AnimatePresence mode="wait">
                    {isAddingToCart ? <motion.div key="loading" initial={{
                    opacity: 0
                  }} animate={{
                    opacity: 1
                  }} exit={{
                ...
                <Button variant={isInWishlist(product.id) ? "secondary" : "outline"} className={cn("w-10 md:w-12 flex justify-center", isInWishlist(product.id) && "text-red-500 bg-red-50 hover:bg-red-100")} onClick={handleToggleWishlist}>
