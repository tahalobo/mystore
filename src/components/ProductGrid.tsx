                  ...
                  <div className="flex items-center mt-1 mb-2">
                    <div className="flex text-amber-400 mr-1">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`h-4 w-4 ${i < product.rating ? "fill-amber-400" : ""}`}
                        />
                      ))}
                    </div>
      ...
      {products.map((product, index) => (
        ...
        >
          <Link to={`/product/${product.id}`} className="block relative">
            ...
            {product.discount && (
              <Badge variant="destructive" className="absolute top-1 right-1 text-xs rounded-full px-1.5 py-0.5">
                -{product.discount}%
              </Badge>
            )}
          </Link>
          ...
          <div className="p-2 flex flex-col flex-1">
            ...
            <div className="flex items-center justify-between mt-2 pt-1 border-t">
              ...
              <div className="flex gap-1">
                ...
                >
                  <Heart
                    className={`h-3.5 w-3.5 ${isInWishlist(product.id) ? "fill-red-500 text-red-500" : ""}`}
                  />
                </button>
